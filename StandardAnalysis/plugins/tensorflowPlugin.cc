...
class tensorflowPlugin : public edm::one::EDAnalyzer<> {
public:
  explicit tensorflowPlugin(const edm::ParameterSet&);
  ~tensorflowPlugin(){};
...
void tensorflowPlugin::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
tensorflowPlugin::tensorflowPlugin(const edm::ParameterSet& config)
...
void tensorflowPlugin::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  // define a tensor and fill it with range(10)
  tensorflow::Tensor input(tensorflow::DT_FLOAT, {1, 145});
...
DEFINE_FWK_MODULE(tensorflowPlugin);
