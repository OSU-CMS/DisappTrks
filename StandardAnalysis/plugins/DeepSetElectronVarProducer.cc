#include "DisappTrks/StandardAnalysis/plugins/DeepSetElectronVarProducer.h"
...
DeepSetElectronVarProducer::DeepSetElectronVarProducer(const edm::ParameterSet &cfg, const CacheData* cacheData) :
...
DeepSetElectronVarProducer::~DeepSetElectronVarProducer()
{
}

std::unique_ptr<CacheData> DeepSetElectronVarProducer::initializeGlobalCache(const edm::ParameterSet& config) {
...
void DeepSetElectronVarProducer::globalEndJob(const CacheData* cacheData) {
  // reset the graphDef
...
void DeepSetElectronVarProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
int
DeepSetElectronVarProducer::getDetectorIndex(const int detectorIndex) const
{
...
void
DeepSetElectronVarProducer::findMatchedIsolatedTrack (const edm::Handle<vector<pat::IsolatedTrack> > &isolatedTracks, edm::Ref<vector<pat::IsolatedTrack> > &matchedIsolatedTrack, double &dRToMatchedIsolatedTrack, const CandidateTrack &track) const
{
...
void
DeepSetElectronVarProducer::findMatchedGenTrack (const edm::Handle<vector<reco::Track> > &genTracks, edm::Ref<vector<reco::Track> > &matchedGenTrack, double &dRToMatchedGenTrack, const CandidateTrack &track) const
{
...
void
DeepSetElectronVarProducer::getGeometries(const edm::EventSetup &setup) {
...
int
DeepSetElectronVarProducer::countGoodPrimaryVertices(const vector<reco::Vertex> &vertices) const
{
  int n = 0;
...
int
DeepSetElectronVarProducer::countGoodJets(const vector<pat::Jet> &jets) const
{
  int n = 0;
...
double
DeepSetElectronVarProducer::getMaxDijetDeltaPhi(const vector<pat::Jet> &jets) const
{
  vector<double> goodJetPhis;
  for(const auto &jet : jets) {
...
double
DeepSetElectronVarProducer::getLeadingJetMetPhi(const vector<pat::Jet> &jets, const pat::MET &met) const
{
...
void
DeepSetElectronVarProducer::getTracks(const edm::Handle<vector<CandidateTrack> > tracks, 
...
void 
DeepSetElectronVarProducer::getRecHits(const edm::Event &event)
{
  recHitInfos_.clear();
...
void
DeepSetElectronVarProducer::getGenParticles(const reco::CandidateView &genParticles){
...
DeepSetElectronVarProducer::getPosition(const DetId& id) const
{
...
const math::XYZVector
DeepSetElectronVarProducer::getPosition(const CSCSegment& seg) const
{
...
const math::XYZVector
DeepSetElectronVarProducer::getPosition(const CSCRecHit2D& recHit) const
{
...
const math::XYZVector
DeepSetElectronVarProducer::getPosition(const DTRecSegment4D& seg) const
{
...
const math::XYZVector
DeepSetElectronVarProducer::getPosition(const DTRecHit1D& recHit) const
{
  const LocalPoint localPos = recHit.localPosition();
...
const math::XYZVector
DeepSetElectronVarProducer::getPosition(const RPCRecHit& seg) const
{
...
vector<pat::Electron>
DeepSetElectronVarProducer::getTagElectrons(const edm::Event &event,
...
vector<pat::Muon>
DeepSetElectronVarProducer::getTagMuons(const edm::Event &event,
                                       const edm::TriggerResults &triggers,
                                       const vector<pat::TriggerObjectStandAlone> &trigObjs,
                                       const reco::Vertex &vertex,
                                       const vector<pat::Muon> &muons)
...
const bool
DeepSetElectronVarProducer::isProbeTrack(const TrackInfo info) const
{
  if(info.pt <= 30 ||
     fabs(info.eta) >= 2.1 ||
...
const unsigned int
DeepSetElectronVarProducer::isTagProbeElePair(const CandidateTrack &probe, const pat::Electron &tag) const 
{
...
const unsigned int
DeepSetElectronVarProducer::isTagProbeTauToElePair(const CandidateTrack &probe, 
...
const unsigned int
DeepSetElectronVarProducer::isTagProbeMuonPair(const CandidateTrack &probe, const pat::Muon &tag) const 
{
  TLorentzVector t(tag.px(), tag.py(), tag.pz(), tag.energy());
  TLorentzVector p(probe.px(), 
                   probe.py(), 
...
const unsigned int
DeepSetElectronVarProducer::isTagProbeTauToMuonPair(const CandidateTrack &probe, 
                                                   const pat::Muon &tag, 
                                                   const pat::MET &met) const 
{
  double dPhi = deltaPhi(tag.phi(), probe.phi());
...
const double
DeepSetElectronVarProducer::minDRBadEcalChannel(const CandidateTrack &track) const
{
...
void
DeepSetElectronVarProducer::getChannelStatusMaps()
{
...
DEFINE_FWK_MODULE(DeepSetElectronVarProducer);
