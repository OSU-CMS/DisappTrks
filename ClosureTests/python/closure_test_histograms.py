...
class ClosureTestHistograms:
    """Loads histograms for the MC lepton background closure test.

    Differences from LeptonBackgroundHistograms (data estimate):
    - Tag selection uses Pt35NoJetCuts instead of Pt55 (lower pT, no jet cuts,
      matching the MC closure test CMSSW jobs)
    - nProbesPassingVeto is read from the base ZtoEleProbeTrk plotter rather
      than from a separate WithFilter job. EventTPProducer computes this variable
      in every event regardless of the doFilter flag, so the result is identical.
    - metNoMu (SS background for the veto numerator) is set to None, skipping
      SS subtraction. This is negligible in clean DY/TTbar MC.
    """
    ...
    def _get_hists_raw(self, nlayers):
        ...
        hists["pass_veto"] = {
            ...
            "passing_veto_probes": get_hist(f, f"{probe}NLayers{nlayers_str}Plotter/Eventvariable Plots/nProbesPassingVeto"),
            "metNoMu":            None,
        }
        hists["pass_met_cut"] = {
        ...
        if self._combined_hists is None:
            ...
            for category in hists_4:
                ...
                for hist_name in hists_4[category]:
                    if hists_4[category][hist_name] is None:
                        combined[category][hist_name] = None
                        continue
