...
G4ClassificationOfNewTrack StackingAction::ClassifyNewTrack(const G4Track * aTrack) 
{
  ...
  if (aTrack->GetCreatorProcess()==0 || aTrack->GetParentID()==0) {
    ...
    */
	...
	if (!savePDandCinTracker && isThisRegion(reg,trackerRegions)) toBeChecked = false;
	if (!savePDandCinCalo    && isThisRegion(reg,caloRegions))    toBeChecked = false;
	if (!savePDandCinMuon    && isThisRegion(reg,muonRegions))    toBeChecked = false;
	if (toBeChecked) {
	  flag = isItPrimaryDecayProductOrConversion(aTrack, *mother);
	}
	if (0 == flag && saveFirstSecondary) flag = isItFromPrimary(*mother, flag);
	//orig
