#include "DisappTrks/SignalMC/plugins/TrackImageAnalyzer.h"
...
TrackImageAnalyzer::TrackImageAnalyzer(const edm::ParameterSet &cfg) :
...
TrackImageAnalyzer::~TrackImageAnalyzer()
{
}

void
TrackImageAnalyzer::analyze(const edm::Event &event, const edm::EventSetup &setup)
{
...
const double
TrackImageAnalyzer::getTrackIsolation(const reco::Track &track, const vector<reco::Track> &tracks) const
{
  double sumPt = 0.0;
...
TrackImageAnalyzer::getPosition(const DetId& id) const
{
   if ( ! caloGeometry_.isValid() ||
...
DEFINE_FWK_MODULE(TrackImageAnalyzer);
