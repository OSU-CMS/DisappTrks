  ...
  cutPythia8Flag_   (cfg.getUntrackedParameter<bool>("cutPythia8Flag", false))
{
  ...
  oneDHists_["genPtPion"] = fs_->make<TH1D>("genPtPion", ";generator pion p_{T} [GeV]", 500, 0.0, 10.0);
  oneDHists_["genPPion"] = fs_->make<TH1D>("genPPion", ";generator pion p [GeV]", 500, 0.0, 10.0);

  oneDHists_["genMet"] = fs_->make<TH1D>("genMet", ";generator MET [GeV]", 1000, 0.0, 2000.0);

  oneDHists_["ewkinoSumPt"] = fs_->make<TH1D>("ewkinoSumPt", ";p_{T}(#chi^{#pm}#chi^{#pm}/#chi^{#pm}#chi^{0}) [GeV]", 1000, 0, 2000);
...
CharginoValidator::analyze(const edm::Event &event, const edm::EventSetup &setup)
{
  ...
  for(const auto &genParticle : *genParticles)
    {
      if(abs(genParticle.pdgId()) != 1000024) continue;
      if(genParticle.numberOfDaughters() == 1) continue;
      if(cutPythia8Flag_ && !genParticle.fromHardProcessBeforeFSR()) continue;
  ...
  for(const auto &genParticle : *genParticles) {
    ...
    if(pdgId != 1000022 && pdgId != 1000024) continue;

    if(cutPythia8Flag_) {
      if(pdgId == 1000022 && status != 23) continue;
      if(pdgId == 1000024 && status != 22) continue;
    }
    else {
  ...
  if(nEwkinos == 2) oneDHists_.at("ewkinoSumPt")->Fill(p.Pt());
