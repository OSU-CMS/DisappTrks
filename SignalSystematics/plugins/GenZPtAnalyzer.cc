...
GenZPtAnalyzer::GenZPtAnalyzer(const edm::ParameterSet &cfg) :
  ...
  is94X_(cfg.getParameter<bool>("is94X"))
{
  TH1::SetDefaultSumw2();
...
GenZPtAnalyzer::getFinalZ(const edm::View<reco::GenParticle> &genParticles) const
{
  ...
  for(int i = is94X_ ?(int)(genParticles.size() - 1) : min(20,(int)(genParticles.size() - 1)); i >= 0; i--) {
    const reco::GenParticle *genParticle = &genParticles.at(i);
    if(!isFinalZ(*genParticle)) continue;
    finalZ = genParticle;
    break;
  }
  return finalZ;
