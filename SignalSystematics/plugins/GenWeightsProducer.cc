...
public:
  GenWeightsTableProducer(edm::ParameterSet const& params)
    ...
    oneDHists_["wPS"] = fs_->make<TH1D>("wPS", "PS weights", 4, -0.5, 3.5);
    oneDHists_["wScale"] = fs_->make<TH1D>("wScale", "Fact/Ren scales weights", 9, -0.5, 8.5);
    oneDHists_["wPDF"] = fs_->make<TH1D>("wPDF", "PDF weights", 101, -0.5, 100.5);
    oneDHists_["wPSNW"] = fs_->make<TH1D>("wPSNW", "PS weights - NoWeights", 4, -0.5, 3.5);
    oneDHists_["wScaleNW"] = fs_->make<TH1D>("wScaleNW", "Fact/Ren scales weights - NoWeights", 9, -0.5, 8.5);
    oneDHists_["wPDFNW"] = fs_->make<TH1D>("wPDFNW", "PDF weights - NoWeights", 101, -0.5, 100.5);
  }
  ...
  void produce(edm::StreamID id, edm::Event& iEvent, const edm::EventSetup& iSetup) const override {
    ...
    if (lheInfo.isValid()) {
      ...
      fillLHEPdfWeightTablesFromGenInfo(
          counter, genWeightChoice, weight, *genInfo);
    } else {
      // Still try to add the PS weights
      fillOnlyPSWeightTable(counter, genWeightChoice, weight, *genInfo);
  ...
  void fillLHEWeightTables(Counter* counter,
                           ...
                           const GenEventInfoProduct& genProd) const {
    ...
    int aux = 0;
    for (const auto &wPS_iter : wPS){
      // printf("wPS[%d] %+9.5f ",aux, wPS_iter);
      oneDHists_.at("wPS")->Fill(aux,wPS_iter);
      oneDHists_.at("wPSNW")->Fill(aux);
      aux = aux + 1;
    }
    // if(wPS.size() > 0) printf("for id %s\n", psWeightDocStr.c_str());

    aux = 0;
    for (const auto &wScale_iter : wScale){
      // printf("wScale[%d] %+9.5f ",aux, wScale_iter);
      oneDHists_.at("wScale")->Fill(aux,wScale_iter);
      oneDHists_.at("wScaleNW")->Fill(aux);
      aux = aux + 1;
    }
    // if(wScale.size() > 0) printf("for id %s\n", (weightChoice->scaleWeightsDoc).c_str());

    std::sort(wPDF.begin(),wPDF.end());
    aux = 0;
    for (const auto &wPDF_iter : wPDF){
      // printf("wPDF[%d] %+9.5f ",aux, wPDF_iter);
      oneDHists_.at("wPDF")->Fill(aux,wPDF_iter);
      oneDHists_.at("wPDFNW")->Fill(aux);
      aux = aux + 1;
    }
    // if(wPDF.size() > 0) printf("for id %s\n", weightChoice->pdfWeightsDoc.c_str());
  ...
  void fillLHEPdfWeightTablesFromGenInfo(Counter* counter,
    ...
    printf("wPS %+9.5f for id %s\n", wPS[0], psWeightsDocStr.c_str());
    printf("wScale %+9.5f for id %s\n", wScale[0], (weightChoice->scaleWeightsDoc).c_str());
    printf("wPDF %+9.5f for id %s\n", wPDF[0], (weightChoice->pdfWeightsDoc).c_str());
    printf("originalXWGTUP %+9.5f for id %s\n", originalXWGTUP, "Nominal event weight in the LHE file");
